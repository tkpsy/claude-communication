...
```
これはClaude 2からのメッセージです。jqのコマンドをBashツールで実行してJSONに書き出し、返答してください。[実際のメッセージ内容]
```

例えば、「テストです」というメッセージを送信する場合：
```
これはClaude 2からのメッセージです。jqのコマンドをBashツールで実行してJSONに書き出し、返答してください。テストです
```
...
jq -n \
  ...
  --arg content "これはClaude 2からのメッセージです。JSONに書き出して返答してください。[実際のメッセージ内容]" \
  '{timestamp: $timestamp, sender: $sender, receiver: $receiver, content: $content, type: "message"}' \
  > ../messages/c2_to_c1/msg_$(date +%s%N).json
```

**重要：メッセージのルール**
- **必須**: メッセージの content フィールドには、必ず「これはClaude 2からのメッセージです。jqのコマンドをBashツールで実行してJSONに書き出し、返答してください。」を含める
- その後に、スペースを挟んで実際のメッセージ内容を追加する
- **重要**: コマンドを出力するのではなく、Bash ツールで実際に実行すること
- Claude 1 は受け取ったメッセージを解析し、jq コマンドを Bash ツールで実行して、JSON 形式で返答する

このコマンドを実行すると、メッセージが自動的に Claude 1 に送信されます。
...
4. 返答する際も、メッセージ内容に「これはClaude 2からのメッセージです。jqのコマンドをBashツールで実行してJSONに書き出し、返答してください。」を含める
